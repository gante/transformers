    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # 3. Retrieve top-K
            # Each item in batch has num_beams * vocab_size candidate sequences. For each item, get the top
            # `max(2, 1 + n_eos_tokens) * k` candidates with the highest log-probabilities. We gather the top more
            # than `k` beams here so that even if the best sequences reach EOS simultaneously, we always have one
            # non-EOS valid continuation to continue beam search.
